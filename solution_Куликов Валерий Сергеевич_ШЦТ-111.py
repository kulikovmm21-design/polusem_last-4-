...
def calculate_daily_ots(df):
    """
    Считаем daily_ots = Weight * count_rows согласно п.5 условия.

    ИСПРАВЛЕНИЕ: вместо weight='first' берём weight='max', чтобы корректно
    обработать редкие случаи (692 группы), когда у одного респондента
    в один день для одного бренда встречается несколько значений Weight.
    Weight — дневной вес респондента; при расхождении берём наибольший,
    чтобы не занижать вклад.
    """
    grouped = df.groupby(
        ...
        weight=('Weight', 'max')   # ИСПРАВЛЕНО: max вместо first
    )
...
def build_clean_ots(df_ots, anomalies_final):
    """
    Вспомогательная функция: возвращает df_ots без аномальных (SubjectID, researchdate).
    Используется как в generate_all_plots, так и в аналитических функциях.
    """
    anom_keys = set(
        anomalies_final['SubjectID'].astype(str) + "_" + anomalies_final['researchdate'].astype(str)
    )
    ots_keys = df_ots['SubjectID'].astype(str) + "_" + df_ots['researchdate'].astype(str)
    return df_ots[~ots_keys.isin(anom_keys)].copy()


# === VISUALIZER (Пункт 8.1) ===
def generate_all_plots(df_ots, anomalies_df, df_clean=None):
    os.makedirs(PLOTS_DIR, exist_ok=True)
    
    if df_clean is None:
        df_clean = build_clean_ots(df_ots, anomalies_df)
...
# =====================================================================
# Функции доступны после запуска main() в интерактивном режиме:
#   python3 -i "solution_Куликов Валерий Сергеевич_ШЦТ-111.py"
# После завершения main() доступны глобальные переменные:
#   _df_raw, _df_ots, _df_clean, _anomalies_final
#
# Примеры вызовов:
#   аналитика_график_до_после_по_признаку(_df_ots, _df_clean, 'Пол')
#   аналитика_таблица_запросов_респондента(_df_raw, 123456, '2025-07-01')
#   аналитика_тренд_бренда_до_после(_df_ots, _df_clean, 'samsung')
...
def аналитика_график_до_после_по_признаку(df_ots, df_clean, имя_колонки):
    ...
    Поддерживает: Пол, Возраст, Регион, Федеральный_округ, ResourceName, ResourceType,
    Platform, UseType, CategoryNameDelivery, Category1, Category2, Category3.

    ИСПРАВЛЕНИЕ: функция теперь работает как с агрегированным df_ots (содержит
    CategoryNameDelivery, Brand и т.д.), так и с сырым df_raw (содержит демографию
    и характеристики ресурсов). Если колонка не найдена в df_ots — ищем в df_raw
    и мёржим через (SubjectID, researchdate).
    """
    if имя_колонки in df_ots.columns:
        before = df_ots.groupby(имя_колонки)['daily_ots'].sum()
        after = df_clean.groupby(имя_колонки)['daily_ots'].sum()
    else:
        print(f"Колонка '{имя_колонки}' отсутствует в агрегированном датасете.")
        print("Попробуйте передать df_raw как df_ots и вызвать функцию снова,")
        print("или воспользуйтесь аналитика_график_до_после_по_признаку_raw().")
        return
...
def аналитика_график_до_после_по_признаку_raw(df_raw, df_ots, df_clean, имя_колонки):
    """
    Версия для демографических и ресурсных признаков из сырых данных
    (Пол, Возраст, Регион, Федеральный_округ, ResourceName, ResourceType, Platform, UseType).
    Мёржит df_ots / df_clean с df_raw по (SubjectID, researchdate).
    """
    if имя_колонки not in df_raw.columns:
        print(f"Колонка '{имя_колонки}' не найдена ни в df_ots, ни в df_raw.")
        return

    # Берём нужные колонки из сырого датасета (уникальные значения на уровне субъект-день-признак)
    attr = df_raw[['SubjectID', 'researchdate', имя_колонки]].drop_duplicates(
        subset=['SubjectID', 'researchdate', имя_колонки]
    )

    merged_before = df_ots.merge(attr, on=['SubjectID', 'researchdate'], how='left')
    merged_after  = df_clean.merge(attr, on=['SubjectID', 'researchdate'], how='left')

    before = merged_before.groupby(имя_колонки)['daily_ots'].sum()
    after  = merged_after.groupby(имя_колонки)['daily_ots'].sum()
...
def аналитика_таблица_запросов_респондента(df_raw, target_subject_id, target_date):
    ...
    """
    res = df_raw[
        (df_raw['SubjectID'] == target_subject_id) &
        (df_raw['researchdate'].astype(str) == str(target_date))
    ]
    if res.empty:
        print(f"Запросы для респондента {target_subject_id} на дату {target_date} не найдены.")
        return res
    cols = [c for c in ['SubjectID', 'researchdate', 'Brand', 'QueryText', 'Weight'] if c in res.columns]
...
def аналитика_тренд_бренда_до_после(df_ots, df_clean, target_brand_name):
    ...
    if b_before.empty:
        print(f"Бренд '{target_brand_name}' не найден в данных.")
        return
...
def main():
    """
    ИСПРАВЛЕНИЕ: main() теперь сохраняет ключевые датафреймы в глобальные переменные
    (_df_raw, _df_ots, _df_clean, _anomalies_final), чтобы аналитические функции
    были доступны после запуска в интерактивном режиме (python3 -i ...).
    Ранее df_clean не определялся нигде вне generate_all_plots, что приводило
    к NameError при попытке вызвать любую из аналитика_* функций.
    """
    global _df_raw, _df_ots, _df_clean, _anomalies_final

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(PLOTS_DIR, exist_ok=True)
    
    print("Загрузка данных...")
    _df_raw = load_and_clean_data()

    print("Расчёт daily_ots...")
    _df_ots = calculate_daily_ots(_df_raw)

    print("Детектирование аномалий...")
    detected_all = run_anomaly_detection(_df_ots)
    
    anomalies_detected = detected_all[detected_all['is_anomaly']]
    _anomalies_final = anomalies_detected[['SubjectID', 'researchdate']].drop_duplicates().reset_index(drop=True)

    # Строим очищенный датасет (нужен для аналитики и графиков)
    _df_clean = build_clean_ots(_df_ots, _anomalies_final)

    # Сохраняем anomalies.csv
    _anomalies_final.to_csv(os.path.join(OUTPUT_DIR, 'anomalies.csv'), index=False)

    # Сохраняем anomaly_reasons.csv
    ...
    generate_all_plots(_df_ots, _anomalies_final, df_clean=_df_clean)
    
    n_anom  = _anomalies_final['SubjectID'].nunique()
    n_total = _df_ots['SubjectID'].nunique()
    ots_kept = _df_clean['daily_ots'].sum() / _df_ots['daily_ots'].sum() * 100

    print(f"\nПайплайн успешно завершён. Результаты сохранены в папке '{OUTPUT_DIR}/'.")
    print(f"Аномальных респондентов: {n_anom} из {n_total} ({n_anom/n_total*100:.1f}%)")
    print(f"OTS сохранено после очистки: {ots_kept:.1f}%")
    print()
    print("Для интерактивного анализа запустите:")
    print('  python3 -i "solution_Куликов Валерий Сергеевич_ШЦТ-111.py"')
    print("Затем используйте функции:")
    print("  аналитика_график_до_после_по_признаку_raw(_df_raw, _df_ots, _df_clean, 'Пол')")
    print("  аналитика_тренд_бренда_до_после(_df_ots, _df_clean, 'samsung')")
    print("  аналитика_таблица_запросов_респондента(_df_raw, <SubjectID>, '<дата>')")
